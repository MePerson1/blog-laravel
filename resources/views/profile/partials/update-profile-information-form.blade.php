<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-400">
            {{ __("Zmien swoją nazwe oraz email.") }}
        </p>
    </header>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-gray-200">
                        {{ __('Twój email nie został zweryfikowany pomyślnie!') }}
                        ...
                        <button form="send-verification" class="underline text-sm text-gray-400 hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800">
                            {{ __('Nacisnij tutaj aby spróbować zweryfikować ponownie.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 font-medium text-sm text-green-400">
                            {{ __('Nowy link weryfikacji zostal wyslany do ciebie na email!') }}
                        </p>
                    @endif
                </div>
            @endif
